    ...
    Dispatch(303,4) {
        ...
        Object massage(Object param) {
            ...
            if ( param instanceof Proxy ) {
                    obj = Proxy.getInvocationHandler(param);
            }
            */
            int ptr = 0;
            /*
            if ( obj instanceof Dispatch ) {
            	ptr = ((IDispatch)obj).getPtr();
            	Native.addRef(ptr);
            	return ptr;
            }
            else {
            */
            	ptr = COM4J.unwrap((com4j.Com4jObject)obj).getPtr();
            //}
            
            int disp = 0;
            if (ptr != 0)
            	disp = COM4J.queryInterface( ptr, COM4J.IID_IDispatch );
        ...
        Object unmassage(Class<?> type, Type genericSignature, Object param) {
            ...
            /*
            if ( IDispatch.class.isAssignableFrom(type) ) {
            	
            	Class<? extends Com4jObject> itf = (Class<? extends Com4jObject>) type;
                Com4jObject r = Wrapper.create(itf, Native.queryInterface(disp, COM4J.IID_IDispatch) );
                Native.release( disp );
                return r;
            }
            */
