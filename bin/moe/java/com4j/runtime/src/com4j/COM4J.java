...
public abstract class COM4J {
    ...
    /**
     * Creates a new Dispatch wrapper for a java object
     *
     */
    
    public static<T>
    com4j.Dispatch<T> createDispatch( Class<T> clazz, T obj ) throws ComException {

    	return new CreateDispatchTask<T>(clazz,obj).execute();
    }


    private static class CreateDispatchTask<T> extends Task<com4j.Dispatch<T>> {

    	private final T obj;
    	private final Class<T> clazz;
        
        public CreateDispatchTask(Class<T> c, T o) {
            this.obj = o;
            this.clazz = c;
        }

        public com4j.Dispatch<T> call() {
        	com4j.Dispatch<T> proxy = new com4j.Dispatch<T>( clazz, obj );
            proxy.nativeProxy = Native.createDispatch( proxy );
            return proxy;
        }
    }
    
    /**
     * Creates a new Dispatch wrapper and wraps it into a com4j.jObject 
     *    	
     */
    
    public static<T >
    com4j.Com4jObject dispatch( Class<T> clazz, T obj ) throws ComException {

    	return new CreateDispatchWrapperTask<T>(clazz,obj).execute();
    }


    private static class CreateDispatchWrapperTask<T> extends Task<com4j.Com4jObject> {

    	private final T obj;
    	private final Class<T> clazz;
        
        public CreateDispatchWrapperTask(Class<T> c, T o) {
            this.obj = o;
            this.clazz = c;
        }
        ...
        public com4j.Com4jObject call() {
        	com4j.Dispatch<T> proxy = new com4j.Dispatch<T>( clazz, obj );
            proxy.nativeProxy = Native.createDispatch( proxy );            
            return Wrapper.create(proxy.nativeProxy);            
        }
    }    
    
    /**
     * Creates a new Dispatch wrapper and wraps it into a com4j.Variant
     *    	
     */   
    
    public static<T >
    com4j.Variant variant( Class<T> clazz, T obj ) throws ComException {

    	return new CreateDispatchVariantTask<T>(clazz,obj).execute();
    }


    private static class CreateDispatchVariantTask<T> extends Task<com4j.Variant> {

    	private final T obj;
    	private final Class<T> clazz;
        
        public CreateDispatchVariantTask(Class<T> c, T o) {
            this.obj = o;
            this.clazz = c;
        }
        ...
        public com4j.Variant call() {
        	...
        	v.set( COM4J.queryInterface( proxy.nativeProxy, COM4J.IID_IDispatch) );
    ...
    /**
     * Gets an interface from GIT
     *
     * @param primaryInterface
     *      The returned COM object is returned as this interface.
     * @param cookie
     *      The GIT cookie to retrieve this interface 
     *
     * @throws ComException
     *      if the retrieval fails.
     *
     */
    public static <T extends Com4jObject> T getInterfaceFromGIT(Class<T> primaryInterface, int cookie ) {
        return new GetGITObjectTask<T>(cookie,primaryInterface).execute();
    }

    private static class GetGITObjectTask<T extends Com4jObject> extends Task<T> {
		private final int cookie;
        //private final GUID iid;
        private final Class<T> intf;

        public GetGITObjectTask( int c, Class<T> intf) {
            //this.clsid = clsid;
			this.cookie = c;
            this.intf = intf;
        }
        ...
        public T call() {
            GUID iid = getIID(intf);
            int ptr = Native.getGITObject( cookie, iid.v[0], iid.v[1] );
            //Native.release(o1);
            return Wrapper.create(intf,ptr);
        }
    }
