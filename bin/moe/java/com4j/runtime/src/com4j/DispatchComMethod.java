...
final class DispatchComMethod extends ComMethod {
    ...
    DispatchComMethod( Method m ) {
        ...
        DISPID id = getDISPID(m);//m.getAnnotation(DISPID.class);
        if(id ==null)
    ...
    public static DISPID getDISPID( Method m) {
    	
        DISPID id = m.getAnnotation(DISPID.class);
        if(id !=null)
        	return id;
        
        Class supe = m.getDeclaringClass().getSuperclass();       
        while (supe != null) {
        	        	
	        if ( supe.equals( java.lang.Object.class ) ) {
	        	break;
	        }
	        	
        	try {
        		Method base = supe.getMethod( m.getName(), m.getParameterTypes());
            	if ( base != null ) {
            		return getDISPID(base);
            	}
        	}
        	catch (Throwable t) {
        	}
