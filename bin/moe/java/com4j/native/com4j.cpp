...
JNIEXPORT jint JNICALL Java_com4j_Native_createDispatch(
	JNIEnv* env, jclass __unused__, jobject obj) {

	//CEventReceiver* p = CEventReceiver::create( env, obj );
	CJDispatch* p = CJDispatch::create(env, obj );
	return reinterpret_cast<jint>(p);	
}
...
(JNIEnv* env, jclass __unused__, jint cookie, jlong iid1, jlong iid2) {
	...
	void* nakedInterfacePtr;
	hr = git->GetInterfaceFromGlobal( (DWORD)cookie, iid, &nakedInterfacePtr );

	git->Release();

	if ( hr == S_OK )
		return (jint)nakedInterfacePtr;
