...
{
	return mol::Thread::call( new Call<B>(bound) );
}
...
#define make_threaded_call( f, ... ) \
	mol::fun::thread_prepare_call( boost::bind( f, __VA_ARGS__ ));

#define make_threaded_callback( f, cb, ... ) \
	mol::fun::thread_prepare_callback( boost::bind( f, __VA_ARGS__ ) , cb );
...
#define threaded_fun_call( f, ... )					\
	mol::Thread::call(								\
		mol::fun::thread_prepare_call(				\
