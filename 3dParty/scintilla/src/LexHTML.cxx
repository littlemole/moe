...
static bool isMasonScriptEnd( unsigned int i, int length,  Accessor &styler, char* masonBuf) {
...
static bool isMasonTag( unsigned int i, int length,  Accessor &styler, char* masonBuf) {
	for ( int x = 0; x < 2; x++ ) {
...
static void ColouriseHyperTextDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[],
	...
	for (int i = startPos; i < lengthDoc; i++) {
		...
		else if ( (!isCommentASPState(state) && (ch == '<') && ((chNext == '%')||(chNext == '&')) && !isPHPStringState(state)) &&
			(!isMasonTag( i+2, length, styler, masonBuf )) ) 
		{
			styler.ColourTo(i - 1, StateToPrint);
			beforePreProc = state;
			if (inScriptType == eNonHtmlScript)
		...
		// handle the end of a pre-processor = Non-HTML
		             ...
		             ((inScriptType == eNonHtmlPreProc)
		              || (inScriptType == eNonHtmlScriptPreProc)) && (
					  ((scriptLanguage != eScriptNone) && stateAllowsTermination(state) && ((ch == '%') || (ch == '?')|| (ch == '&')))
		             ) && ( 
					     (chNext == '>') ||
						 ( chPrev == '/'  && isMasonScriptEnd(i+1,length,styler,masonBuf))
					 ) ||
